class HomeController < ApplicationController
  def index
    @latest_quotes = Quote.all(:conditions => ['NOT hidden'], :order => 'created_at desc', :limit => 5)
    ...
    if logged_in?
      ...
      @current_user_quotes = Quote.all(:conditions => ['context_id in (?) AND NOT hidden', current_user.context_ids], :order => 'created_at desc', :limit => 5)
